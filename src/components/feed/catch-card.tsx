...
export function CatchCard({ post, onDelete }: { post: CatchPost; onDelete?: (id: string) => void }) {
  ...
  async function toggleFollow() {
    ...
    if (!data.user || data.user.id === post.author.id) return;
    if (next) await supabase.from("follows").upsert({ follower_id: data.user.id, following_id: post.author.id });
    else await supabase.from("follows").delete().eq("follower_id", data.user.id).eq("following_id", post.author.id);
  }
  function sharePost() { navigator.share?.({ title: post.title, text: post.story, url: `${window.location.origin}/?post=${post.id}` }); }
  function report(kind: "post" | "angler") { setMenuOpen(false); setPostError(`${kind === "post" ? "Post" : "Angler"} report noted. Moderation tools will review it.`); }
  ...
  return <article className="catch-card surface">
    <div className="catch-head"><Link className="post-author" href={`/profile/${post.author.id}`} aria-label={`View ${post.author.name}'s profile`}><Image src={post.author.avatar} width={44} height={44} alt="" /><span><strong>{post.author.name}</strong><small>{post.author.level} · {post.createdAt}</small></span></Link><div className="post-menu-wrap"><button className="post-menu-button" type="button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Post options" aria-expanded={menuOpen}><MoreHorizontal /></button>{menuOpen && <div className="post-menu"><button type="button" onClick={toggleFollow}>{followed ? "Unfollow angler" : "Follow angler"}</button><button type="button" onClick={sharePost}><Share2 />Share post</button><button type="button" onClick={() => report("post")}>Report post</button><button type="button" onClick={() => report("angler")}>Report angler</button>{ownsPost && <button type="button" onClick={deletePost} disabled={deleting}><Trash2 />{deleting ? "Deleting..." : "Delete post"}</button>}</div>}</div></div>{postError && <p className="post-error" role="alert">{postError}</p>}
